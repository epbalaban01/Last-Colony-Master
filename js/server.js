...
var server = http.createServer(function(request,response){
    ...
    response.end("Bu node.js HTTP sunucusudur.");
});

server.listen(8080,function(){
    console.log('Sunucu 8080 port numaralı bağlantı noktasını bağlanmaya başladı');
});
...
wsServer.on('request',function(request){
	    ...
	    if (message.type === 'utf8') {
	        ...
	        switch (clientMessage.type){
	            ...
	            case "command":
	                ...
	                break;    
				case "lose_game":
					endGame(player.room, ""+ player.color +" Takım yenildi.");                
					break;
				case "chat":
					if (player.room && player.room.status=="running"){
    ...
    connection.on('close', function(reasonCode, description) {
	    ...
	    if(player.room){
	        ...
	        if(status=="running"){                
	            endGame(player.room, ""+ player.color +" oyuncunun bağlantısı kesildi.");                
	        } else {
...
function joinRoom(player,roomId){
    ...
    console.log("Odaya oyuncu ekleme",roomId);
...
function leaveRoom(player,roomId){
    var room = gameRooms[roomId-1];
    console.log("Oynatıcıyı odadan çıkarma",roomId);
...
function initGame(room){
    console.log("Her iki oyuncu da katıldı. Oda için oyun başlatılıyor "+room.roomId);
...
function startGame(room){
    console.log("Her iki oyuncu da hazır. Odada oyun başlama",room.roomId);
    room.status = "running";
    sendRoomListToEveryone();
...
function finishMeasuringLatency(player,clientMessage){
    ...
    console.log("Oyuncu için Gecikme Oldu. Attempt", player.latencyTrips.length, "- Ortalama Gecikme:",player.averageLatency, "<", player.tickLag);
}
