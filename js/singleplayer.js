...
var singleplayer = {
	    ...
	    for (var type in level.requirements){
	           ...
	           for (var i=0; i < requirementArray.length; i++) {
	               ...
	               if (window[type]){
	                   ...
	                   console.log('Türü yüklenemedi :',type);
	               }
	           };
	    };
	...
	endLevel:function(success){
	    ...
	    if (success){
	        ...
	        if (moreLevels){
	            game.showMessageBox("Görev tamamlandı.",function(){
	                $('.gamelayer').hide();
	                singleplayer.currentLevel++;
	                singleplayer.startCurrentLevel();
	            });
	        } else {
	            game.showMessageBox("Görev Tamamlandı. <br> <br> Bu kampanyadaki son görevdi. <br> <br> Oynadığınız için teşekkür ederiz.",function(){
	                $('.gamelayer').hide();
	                $('#gamestartscreen').show();
	            });
	        }
	    } else {
	        game.showMessageBox("Görev başarısız.<br><br>Tekrar Deneyin?",function(){
	            $('.gamelayer').hide();
	            singleplayer.startCurrentLevel();
	        }, function(){
	            $('.gamelayer').hide();
